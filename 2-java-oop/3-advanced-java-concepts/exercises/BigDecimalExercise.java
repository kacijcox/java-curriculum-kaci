...
public class BigDecimalExercise {
    ...
    BigDecimal sum(BigDecimal[] values) {
        //sort through the array of values
    ...
    BigDecimal average(BigDecimal[] values) {
        ...
        BigDecimal resultTwo = result.divide(BigDecimal.valueOf(values.length));

        return resultTwo.setScale(4, RoundingMode.HALF_UP);
        }
    ...
    BigDecimal calculateInterest(BigDecimal investment, BigDecimal interestRate, int periods) {
        ...
        BigDecimal interest = investment.multiply(BigDecimal.ONE.add(interestRate).pow(periods));
        BigDecimal difference = interest.subtract(investment);
